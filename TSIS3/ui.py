...
RED    = (220, 50,  50)
GREEN  = (0,   200, 80)
PINK   = (255, 105, 180) # Added Pink for the title
TEXT_COLOR = (30, 30, 30) # Dark color for text on white background
...
def button(screen, text, x, y, w=200, h=50, color=GRAY):
    ...
    pygame.draw.rect(screen, BLACK, rect, 2, border_radius=8) # Border changed to black
...
def main_menu(screen):
    W, H = screen.get_size()
    while True:
        screen.fill(WHITE) # Background changed to white
        title = font_big.render("RACER", True, PINK) # Title changed to pink
...
def enter_name(screen):
    ...
    while True:
        screen.fill(WHITE) # Background changed to white
        screen.blit(font_med.render("Enter your name:", True, TEXT_COLOR), (W//2-130, 220))
        
        box = pygame.Rect(W//2-100, 270, 200, 45)
        pygame.draw.rect(screen, (230, 230, 230), box, border_radius=6) # Lighter gray box
        pygame.draw.rect(screen, BLACK, box, 2, border_radius=6)
        screen.blit(font_med.render(name + "|", True, PINK), (box.x+8, box.y+8))
...
def leaderboard_screen(screen):
    ...
    while True:
        screen.fill(WHITE) # Background changed to white
        screen.blit(font_big.render("TOP 10", True, PINK), (W//2-80, 30))
        for i, row in enumerate(board):
            txt = f"{i+1}. {row['name']}  {row['score']} pts  {row['distance']}m"
            # Use Pink for #1, Black for others
            color = PINK if i == 0 else TEXT_COLOR
            screen.blit(font_small.render(txt, True, color), (20, 100 + i*40))
...
def settings_screen(screen):
    ...
    while True:
        screen.fill(WHITE) # Background changed to white
        screen.blit(font_big.render("Settings", True, PINK), (W//2-110, 30))

        screen.blit(font_med.render(f"Sound: {'ON' if s['sound'] else 'OFF'}", True, TEXT_COLOR), (20, 130))
        b_snd = button(screen, "Toggle", W-160, 125, w=140, h=40)

        screen.blit(font_med.render(f"Car: {s['car_color']}", True, TEXT_COLOR), (20, 200))
        b_col = button(screen, "Change", W-160, 195, w=140, h=40)

        screen.blit(font_med.render(f"Difficulty: {s['difficulty']}", True, TEXT_COLOR), (20, 270))
...
def game_over_screen(screen, score, distance, coins):
    ...
    while True:
        ...
        screen.blit(font_med.render(f"Score:    {score}",     True, TEXT_COLOR), (W//2-100, 190))
        screen.blit(font_med.render(f"Distance: {distance}m", True, TEXT_COLOR), (W//2-100, 240))
        screen.blit(font_med.render(f"Coins:    {coins}",     True, PINK), (W//2-100, 290))
